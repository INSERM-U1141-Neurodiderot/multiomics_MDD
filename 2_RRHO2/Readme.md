...
For each omic, results from differential analysis in each sex were ranked based on the following metric: -log10(p-value) x sign(log2 Fold Change). Then, the RRHO2par function was applied to the 2 lists at default parameters (with step size equal to the square root of the list length)

#### Data
Using the output *res_single_mRNA.rds* R data object generated by mRNA single omic analysis (cf. line 192 [/1_Single_Omics/README.md](https://github.com/INSERM-U1141-Neurodiderot/multiomics_MDD/blob/main/1_Single_Omics/README.md))
```{r}
